...
int main() {
    ...
    std::string name;
    std::string type;
    std::vector<int> anchors; // 使用 vector 来存储多个 anchors
    int num_anchors = 0;
    ...
    if (keyValueMap.find("anchors") != keyValueMap.end()) {
        std::istringstream(keyValueMap["anchors"]) >> anchors.emplace_back(std::stoi(keyValueMap["anchors"]));
    }
