...
void generate_tensor_2D_2(float *ptr, int i_M, int i_N){        // 二维矩阵填充函数（此处全部填充1）
    for(int i = 0; i < i_M; i++){
        for(int j = 0; j < i_N; j++){
            *(ptr + i*i_N + j ) = 2.0;
        }
...
int main(int argc, const char *arg[]) {
    ...
    generate_tensor_2D_2(C, M, N);     // 填充C矩阵
    // memset(C, 0, C_mem_size);
    ...
    // cudaMemcpy(C, d_C, C_mem_size, cudaMemcpyDeviceToHost);  //将运行结果D矩阵传回host端
